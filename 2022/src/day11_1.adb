...
begin
   ...
   declare
      Most_Active_Count : array (1 .. 2) of Number := (others => 0);
   begin
      for M of All_Monkeys loop
         if M.Inspected > Most_Active_Count (1) then
            Most_Active_Count (2) := Most_Active_Count (1);
            Most_Active_Count (1) := M.Inspected;
         end if;
      end loop;

      Put (Output, Most_Active_Count (1) * Most_Active_Count (2));
      New_Line (Output);
   end;
end Day11_1;
