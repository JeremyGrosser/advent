...
procedure Day7_1 is
   ...
   begin
      ...
      String'Write (Error, "CWD " & To_String (Working_Dir));
      New_Line (Error);
   ...
   begin
      String'Write (Error, "DIR ");
      String'Write (Error, N);
      New_Line (Error);
      Append (P, N);
   ...
   begin
      for I in Iterate (Files) loop
         if Element (I).Is_Dir then
            Dir_Sum := Size (Key (I));
            String'Write (Error, To_String (Key (I)));
            String'Write (Error, " ");
            String'Write (Error, Dir_Sum'Image);
            New_Line (Error);
