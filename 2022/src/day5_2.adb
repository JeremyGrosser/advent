...
procedure Day5_2 is
   ...
   procedure Move_Crates
      (Count, From, To : Positive)
   is
      Crane : Crate_Vectors.Vector := Crate_Vectors.Empty_Vector;
      C : Crate;
   begin
      for I in 1 .. Count loop
         C := First_Element (Reference (Yard, From));
         Delete_First (Reference (Yard, From));
         Append (Crane, C);
      end loop;
      Prepend (Reference (Yard, To), Crane);
   end Move_Crates;
...
begin
   ...
   begin
      while not End_Of_Input loop
         ...
         Move_Crates (Move_Count, From_Column, To_Column);
      end loop;
   end;
